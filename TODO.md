# Billbook TODO

- [ ] Expand finance integrations with Fidelity support
- [ ] Expand finance integrations with Discover support
- [ ] Add photo support to journal entries
- [ ] Improve backwards compatibility for older entry formats and app versions
- [ ] Rework Touch ID / app security lock so it actually protects launch cleanly
- [ ] Fix duplicate macOS Safe Storage password prompts on startup
- [ ] Increase the app font size slightly
- [x] Days of the week
- [ ] Predictions reminders
